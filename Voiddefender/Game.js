...
BunnyDefender.Game.prototype = {
    create: function(){
        ...
        this.timer = this.time.create(false); //we don't want to remove after the game
    ...
    buildWorld: function(){
        ...
        this.buildEmitter();
        this.countdown = this.add.bitmapText(10, 10, 'eightbitwonder', 'Creatures Left ' + this.totalBunnies, 20);
        this.timer.start();
    },
    ...
    assignBunnyMovement: function(b) {
        ...
        t = this.add.tween(b).to({x:bposition}, 3500, Phaser.Easing.Quadratic.InOut, true, bdelay); //properties, duration, ease, autoStart, delay ** but what is tween()?
    ...
    startBunny: function(b) {
        ...
        b.animations.play('Walk', 24, true); // frame, autoloop
    },
    ...
    buildEmitter:function() {
        this.burst = this.add.emitter(0, 0, 80); //80particles
        this.burst.minParticleScale = 0.3; //scale size
        this.burst.maxParticleScale = 1.2;
        this.burst.minParticleSpeed.setTo(-30, 30); //burst effect
    ...
    checkBunniesLeft: function(){
        if (this.totalBunnies <= 0){
            this.gameover = true;
            this.music.stop();
            this.countdown.setText('Creatures Left 0');
            ...
            this.countdown.setText('Creatures Left ' + this.totalBunnies);
        }
    },
